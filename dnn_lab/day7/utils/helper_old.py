...
import logging
from util.log_event import fn_log_event  # type: ignore

from util.configuration import Config  # type: ignore
...
def fn_get_date_taken(path):
    ...
    return Image.open(path)._getexif()[36867]  # type: ignore
...
def get_image_data(imagePath, **kwargs):
    ...
    assert img is not None, "Failed to read image : %s, %s" % (image_id)  # type: ignore
...
def fn_draw_rectangle(img,slotID, xMin, yMin, w, h, free = 1, use_normalized_coordinates = False):
    ...
    try:
        ...
        if platform.dist()[0] == 'Ubuntu':  # type: ignore
    ...
    display_str_heights = font.getsize(displayStr)[1]  # type: ignore
    ...
    text_width, text_height = font.getsize(displayStr)  # type: ignore

    margin = np.ceil(0.05 * text_height)

    draw.rectangle([(left, text_bottom - text_height - 2 * margin),  # type: ignore
